 ...
 * @LastEditTime: 2022-10-22 20:48:20
...
let articles = async function (articleList, tagId) {
  ...
  for (let article of articleList) { //循环文章数组的每一个元素
    ...
    if (Array.isArray(oldArticles) && oldArticles.length >0) {
       await query(`UPDATE articles SET title=?, href=?, likes=?, tag_id=? WHERE id=?`,[article.title, article.href,  article.likes, tagId, article.id]);
    } else {
        //如果走到 这个分支，就意味着读取了新的文章。则认为文章就更新了
       await  query(`INSERT INTO articles(id,title,href,likes,tag_id) VALUES(?,?,?,?,?)`,[article.id, article.title, article.href, article.likes, tagId]);
    }
  }
}
